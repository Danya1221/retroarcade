...
export function render(c, s, color = "#bdff70", settings = {}, skin = {}) {
  ...
  if(s.game==="racer"){
    ...
    for(const a of s.cars)if(a.d>0&&a.d<520)car(a.lane,a.d,"#ffd45f");for(const p of s._remotePlayers||[]){const lane=((Number(p.slot)%4)-1.5)*.36;car(lane,32+Number(p.slot)*13,["#69d7ff","#d58cff","#8cff9b","#ffb45f"][Number(p.slot)%4])}car(s.x,0,"#ef514c");
