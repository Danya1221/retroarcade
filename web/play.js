...
export async function play(session, api, settings, skin, onExit, onResult, toast, multiplayer = null) {
  ...
  function frame(now) {
    ...
    hud.textContent = state.game === "platformer"
      ? `LV ${state.level} · ${String(state.score).padStart(5, "0")} · ♥ ${state.hp} · KEY ${state.keysHeld || 0}`
